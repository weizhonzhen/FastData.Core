...
namespace Data.Core.Check
{
    ...
    internal static class BaseTable
    {
        ...
        public static void Check(DataQuery item, string tableName, string nameSpace, List<PropertyInfo> listInfo, List<Attribute> listAttribute)
        {
            try
            {
                if (item.Config.DesignModel == Data.Core.Base.Config.CodeFirst)
                {
        ...
        private static TableModel GetTable(DataQuery item, string tableName)
        {
            ...
            if (item.Config.DbType == DataDbType.MySql)
            {
                ...
                var dicList = LambdaRead.ExecuteSql(sql, param.ToArray()) ?? new List<Dictionary<string, object>>();
                foreach (var temp in dicList)
                {
                    var model = new ColumnModel();
